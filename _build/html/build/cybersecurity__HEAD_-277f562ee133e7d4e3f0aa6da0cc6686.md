# Cybersecurity

In this section, we will discussing and experimenting with various computer security and ethical hacking topics.  We will also provide brief recaps of cybersecurity talks and conferences attended.

::::{grid} 2 2 2 2

:::{grid-item-card}
:link: Protostar/protostar

Protostar Exploit Exercises
^^^
```{figure} images/Protostar/stack0_title.jpg
:height: 200
:enumerated: false
:class: no-caption clickable-figure


```
:::

:::{grid-item-card}
:link: LiveOverflow/liveoverflow

LiveOverflow - Stack0
^^^
```{figure} images/LiveOverflow/LiveOverflow_main2.jpg
:height: 200
:enumerated: false
:class: no-caption clickable-figure


```
:::

:::{grid-item-card}
:link: Basic_Linux_Exploits/basic_linux_exploits

Basic Linux Exploits
^^^
```{figure} images/Basic_Linux_Exploits/basic_linux_exploits_splash.jpeg
:height: 200
:enumerated: false
:class: no-caption clickable-figure


```
:::

:::{grid-item-card}
:link: Disassembling_Binaries/disassembling_binaries

Disassembling Binaries
^^^
```{figure} images/Disassembling_Binaries/disassembling_binaries_splash.jpg
:height: 200
:enumerated: false
:class: no-caption clickable-figure


```
:::

:::{grid-item-card}
:link: Shellcode_Generation_and_Testing/shellcode_generation_and_testing

Shellcode Generation and Testing
^^^
```{figure} images/Shellcode_Generation/shellcode_generation.jpg
:height: 200
:enumerated: false
:class: no-caption clickable-figure


```
:::

:::{grid-item-card}
:link: SLAE64/SLAE64

Pentester Academy: 
SLAE64 - SecurityTube Linux Assembly Language and Shellcoding Expert 64-Bit
^^^
```{figure} images/SLAE64/SHELLCODING64.png
:height: 200
:enumerated: false
:class: no-caption clickable-figure


```
:::

::::
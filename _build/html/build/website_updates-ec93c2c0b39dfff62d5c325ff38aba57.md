...
## 7/26/2026
- Beej Guide C: complete chapters 5–7 (pointers, arrays, strings) with executable notebooks, technical card images, TOC rebuild; clean rebuild HTML.
- Fortran MOOC: convert remaining section heads with site-wide `{tableofcontents}` (or empty heads) to card TOCs with images; clean rebuild HTML.
...
- Fortran MOOC landing page: topic card grid with free Unsplash images + attribution; clean rebuild HTML.
- Added many Fortran MOOC notebooks from remaining OG course sources (hello, GCD, leap year, types, strings, random, primes, Fibonacci, Kaprekar, coins, Hanoi, etc.) with FPM packages and step-by-step explanations; clean rebuild HTML.
- Restored card thumbs to `{image}` (cards are links again); lightbox only for content `{figure}` images; clean rebuild HTML.
- Made content figures clickable to expand (lightbox); card images stay non-expanding links; clean rebuild HTML.
- Projects polish: enable debug, uncomment valuable prints, annotated plots with in-frame labels, descriptive text; website-scale midterm/final demo data; **Grok Build** now used for site authoring; clean rebuild HTML.
- Fixed doubled page titles (MyST frontmatter title instead of body H1); Project 1 prints/plots executed with outputs; clean rebuild HTML.
...
## 12/31/2025
- Finished the addition of the MCS Portfolio Project.  Added EEE571 Project.
...
## 12/24/2025 
- Continuing process of removing legacy Myst directives and correcting Tables of Contents.

## 12/20/2025
- Added my MCS Portfolio Project.  Numerous fixes to many pages.  Still much to do.

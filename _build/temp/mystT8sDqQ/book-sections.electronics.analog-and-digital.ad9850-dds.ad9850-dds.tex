\paragraph{Variable Frequency Oscillator Using Direct Digital Synthesis}
...
\subparagraph{Circuit Diagram}

The circuit diagram for the variable frequency oscillator is shown below:

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.7\linewidth]{files/AD9850_DDS_Circuit-6f03dc8af3f9897750c6eaddec6f169c.png}
\end{figure}

\subparagraph{Building the Circuit}

To build the circuit, follow these steps:

\begin{enumerate}
\item Connect the AD9850 DDS module to the Arduino Uno as follows:

\begin{itemize}
\item Connect the AD9850 module's VCC pin to the Arduino's 5V pin.
\item Connect the AD9850 module's GND pin to the Arduino's GND pin.
\item Connect the AD9850 module's DATA pin to the Arduino's digital pin 8.
\item Connect the AD9850 module's W\_CLK pin to the Arduino's digital pin 9.
\item Connect the AD9850 module's FQ\_UD pin to the Arduino's digital pin 10.
\item Connect the AD9850 module's RESET pin to the Arduino's digital pin 11.
\end{itemize}


\item Connect the 16x2 LCD display to the Arduino Uno as follows:

\begin{itemize}
\item Connect the LCD display's VCC pin to the Arduino's 5V pin.
\item Connect the LCD display's GND pin to the Arduino's GND pin.
\item Connect the LCD display's SDA pin to the Arduino's analog pin A4.
\item Connect the LCD display's SCL pin to the Arduino's analog pin A5.
\end{itemize}


\item Connect the rotary encoder to the Arduino Uno as follows:

\begin{itemize}
\item Connect the rotary encoder's CLK pin to the Arduino's digital pin 2.
\item Connect the rotary encoder's DT pin to the Arduino's digital pin 3.
\item Connect the rotary encoder's SW pin to the Arduino's digital pin 4.
\end{itemize}


\item Connect the power supply to the circuit. The AD9850 module requires a 5V power supply.
\end{enumerate}

\subparagraph{Programming the Arduino}

To program the Arduino Uno, you will need to install the AD9850 library. The library provides functions for controlling the AD9850 module and generating frequency outputs. You can download the library from the following link: \href{https://github.com/markkhusid/AD9850\_DDS\_VFO}{Project Code}

Once you have installed the library, you can upload the following code to the Arduino Uno:

\subparagraph{Rotary Encoder Handler}
...
\subparagraph{Video of the Project}

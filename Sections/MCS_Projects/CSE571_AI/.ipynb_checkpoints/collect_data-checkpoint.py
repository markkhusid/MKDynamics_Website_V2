...
def collect_training_data(total_actions):
    ...
    for action_i in range(total_actions):
        ...
        flat_data = [*sensor_readings, action, collision]
        network_params.append(flat_data)
        print(f'Network parameters collected: {network_params[action_i]} {len(network_params)}', end="\n", flush=True)
    ...
    with open("training_data_20k.csv", "w", newline="") as csvfile:
        csvwriter = csv.writer(csvfile)
        for row in network_params:
